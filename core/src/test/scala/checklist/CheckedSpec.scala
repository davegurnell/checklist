...
class CheckedSpec extends AnyWordSpec with Matchers {
  ...
  private val valueOnly   = Ior.right[Messages, Int](42)
  private val errorAnd    = Ior.both[Messages, Int](error("Fail"), 42)
  private val warningAnd  = Ior.both[Messages, Int](warning("Warn"), 42)
  private val bothAnd     = Ior.both[Messages, Int](error("Fail") ++ warning("Warn").toList, 42)
  private val errorOnly   = Ior.left[Messages, Int](error("Fail"))
  private val warningOnly = Ior.left[Messages, Int](warning("Warn"))
  private val bothOnly    = Ior.left[Messages, Int](error("Fail") ++ warning("Warn").toList)
