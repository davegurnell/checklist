...
trait RuleHListSyntax {
  ...
  implicit class RuleHList[A, B <: HList, Rev <: HList](rule: Rule[A, B])(implicit reverse: Reverse.Aux[B, Rev]) { self =>
    ...
    /** Adds a new property to the rule builder to be sanitzed and validated. You usually want to use the overload which lets you specify a path, but
      * if you do not want to specify a path, use this one.
      *
      * @param f
    ...
    /** Checks a property and discards the result.
      ...
      *   The raw, unvalidated type which is pulled out of an `A`
      */
    def checkAndDrop[P: PathPrefix, C](path: P, f: A => C)(rule: Rule[C, ?]): Rule[A, B] =
    ...
    /** Checks a property and discards the result.
      ...
      *   The raw, unvalidated type which is pulled out of an `A`
      */
    def checkAndDrop[C](f: A => C)(rule: Rule[C, ?]): Rule[A, B] =
